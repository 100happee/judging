import { onValue, ref, remove, update } from "https://www.gstatic.com/firebasejs/10.12.5/firebase-database.js";
    ...
    <tr data-id="${escapeHtml(submission.id)}">
      ...
      <td>
        <div class="action-stack">
          ...
          <button class="primary-btn compact danger-btn delete-btn" type="button">Delete</button>
        </div>
...
tableBody.addEventListener("click", async (event) => {
  ...
  if (deleteButton) {
    await deleteSubmission(deleteButton);
    return;
  }
...
async function deleteSubmission(button) {
  ...
  const submitter = row.children[1]?.textContent?.trim() || "this submitter";
  const songName = row.children[2]?.textContent?.trim() || "this submission";
  const confirmed = window.confirm(`Delete "${songName}" from ${submitter}? This cannot be undone.`);
  if (!confirmed) return;
  ...
  try {
    ...
    window.alert(`Could not delete submission: ${error.message}`);
  }
}
